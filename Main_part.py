# -*- coding: utf-8 -*-
"""
Created on Fri May  8 14:11:55 2020
...
mask = r'C:\Users\madhu\AppData\Roaming\SPB_Data\Face_Mask\mask'
no_mask = r'C:\Users\madhu\AppData\Roaming\SPB_Data\Face_Mask\no_mask'
...
labels[:n_mask,0] = 1    #few of them are masked there corresponding array value 1
labels[n_mask:,1] = 1    #few of them are non-masked there corresponding array value 1
...
shuffle(index) #To generate random array of value of integers 
index = np.array(index)#convert list into array to perform functions
image =   image[index]
labels =   labels[index,:] #arrange labels accorfing training index size()
x_train =  image[:int(0.8*data_size)]
y_train =  labels[:int(0.8*data_size),:]  #images and there corresponding labels 
x_test =   image[int(0.8*data_size):]
y_test =   labels[int(0.8*data_size):,:]  #correspond test and training tests 
...
#Randomly take one integer and print its label 
